...
The persisted notification policy is sender-local UI state, not part of a Lane or frozen execution snapshot. `/watch` and `/mute` add an in-memory override for the active or next task; the override is deleted on completion, stop, or reset and is intentionally not restored after process restart. Mode changes never alter the Agent, model, access, or workspace selected for the task. Quiet mode suppresses ordinary tool summaries and heartbeats only; durable final replies, failures, and normalized native input/confirmation requests keep their delivery guarantees.

Tool progress uses a provider-neutral bounded accumulator rather than an exhaustive command classifier. Every structured `tool_use` is counted as command, read, search, write, web, or other, with unknown tools taking the generic fallback. Representative samples are redacted, length-bounded, fingerprinted, and deduplicated. Normal mode sends the first significant operation immediately, then at most eight changed 30-second summaries; verbose mode uses bounded five-second summaries. Active tool IDs, samples, unique immediate failures, and outbound progress messages all have fixed caps. A failed `tool_result` is correlated by event ID when possible and delivered immediately even in quiet mode.
